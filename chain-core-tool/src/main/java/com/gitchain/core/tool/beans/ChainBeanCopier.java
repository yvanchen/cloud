 ...
 * @author git
 */
public abstract class ChainBeanCopier {
	private static final Type CONVERTER = TypeUtils.parseType("org.springframework.cglib.core.Converter");
	private static final Type BEAN_COPIER = TypeUtils.parseType(ChainBeanCopier.class.getName());
	...
	/**
	 * The map to store {@link ChainBeanCopier} of source type and class type for copy.
	 */
	private static final ConcurrentMap<ChainBeanCopierKey, ChainBeanCopier> BEAN_COPIER_MAP = new ConcurrentHashMap<>();

	public static ChainBeanCopier create(Class source, Class target, boolean useConverter) {
		return ChainBeanCopier.create(source, target, useConverter, false);
	}

	public static ChainBeanCopier create(Class source, Class target, boolean useConverter, boolean nonNull) {
		ChainBeanCopierKey copierKey = new ChainBeanCopierKey(source, target, useConverter, nonNull);
	...
	public static class Generator extends AbstractClassGenerator {
		private static final Source SOURCE = new Source(ChainBeanCopier.class.getName());
		...
		@Override
		public ChainBeanCopier create(Object key) {
			return (ChainBeanCopier) super.create(key);
		}
		...
		public void generateClass(ClassVisitor v) {
			...
			for (PropertyDescriptor setter : setters) {
				...
				//  2019.01.12 优化逻辑，先判断类型，类型一致直接 set，不同再判断 是否 类型转换
				// nonNull Label
