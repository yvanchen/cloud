 ...
 * @author git 
 */
class ChainBeanMapEmitter extends ClassEmitter {
	private static final Type BEAN_MAP = TypeUtils.parseType(ChainBeanMap.class.getName());
	...
	public ChainBeanMapEmitter(ClassVisitor v, String className, Class type, int require) {
		super(v);
		...
		if (require != 0) {
			for (Iterator it = allProps.keySet().iterator(); it.hasNext(); ) {
				...
				if ((((require & ChainBeanMap.REQUIRE_GETTER) != 0) && !getters.containsKey(name)) ||
					(((require & ChainBeanMap.REQUIRE_SETTER) != 0) && !setters.containsKey(name))) {
					it.remove();
					getters.remove(name);
					setters.remove(name);
				}
