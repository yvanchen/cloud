...
import com.gitchain.core.tool.function.CheckedFunction;
import com.gitchain.core.tool.utils.ClassUtil;
import com.gitchain.core.tool.utils.ConvertUtil;
import com.gitchain.core.tool.utils.ReflectUtil;
import com.gitchain.core.tool.utils.Unchecked;
...
public class ChainConverter implements Converter {
	 ...
	 * @param fieldName 目标的field名，原为 set 方法名，ChainBeanCopier 里做了更改
	 * @return {Object}
	 */
	@Override
	@Nullable
	public Object convert(Object value, Class target, final Object fieldName) {
		...
		try {
			TypeDescriptor targetDescriptor = ChainConverter.getTypeDescriptor(targetClazz, (String) fieldName);
			// 1. 判断 sourceClazz 为 Map
			if (Map.class.isAssignableFrom(sourceClazz)) {
				return ConvertUtil.convert(value, targetDescriptor);
			} else {
				TypeDescriptor sourceDescriptor = ChainConverter.getTypeDescriptor(sourceClazz, (String) fieldName);
				return ConvertUtil.convert(value, sourceDescriptor, targetDescriptor);
			}
		} catch (Throwable e) {
			log.warn("ChainConverter error", e);
			return null;
		}
	}
