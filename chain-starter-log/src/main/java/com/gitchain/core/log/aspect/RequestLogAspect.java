...
@ConditionalOnProperty(value = ChainLogLevel.REQ_LOG_PROPS_PREFIX + ".enabled", havingValue = "true", matchIfMissing = true)
public class RequestLogAspect {

	private final ChainRequestLogProperties properties;
	 ...
	 */
	@Around(
		"execution(!static com.gitchain.core.tool.api.R *(..)) && " +
			"(@within(org.springframework.stereotype.Controller) || " +
			"@within(org.springframework.web.bind.annotation.RestController))"
	)
	public Object aroundApi(ProceedingJoinPoint point) throws Throwable {
		ChainLogLevel level = properties.getLevel();
		// 不打印日志，直接返回
		if (ChainLogLevel.NONE == level) {
		...
		try {
			...
			// 打印返回结构体
			if (ChainLogLevel.BODY.lte(level)) {
	 ...
	 * @param beforeReqArgs beforeReqArgs
	 */
	public void logIngHeaders(HttpServletRequest request, ChainLogLevel level,
							  StringBuilder beforeReqLog, List<Object> beforeReqArgs) {
		// 打印请求头
		if (ChainLogLevel.HEADERS.lte(level)) {
