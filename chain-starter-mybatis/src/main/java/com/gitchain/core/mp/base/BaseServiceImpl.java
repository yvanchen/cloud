package com.gitchain.core.mp.base;
	...
	private void resolveEntity(T entity) {
		...
		if (entity.getId() == null) {
			...
			if (entity.getStatus() == null) {
				entity.setStatus(ChainConstant.DB_STATUS_NORMAL);
			}
		...
		entity.setIsDeleted(ChainConstant.DB_NOT_DELETED);
		// 处理多租户逻辑，若字段值为空，则不进行操作
		Field field = ReflectUtil.getField(entity.getClass(), ChainConstant.DB_TENANT_KEY);
		if (ObjectUtil.isNotEmpty(field)) {
			Method getTenantId = ClassUtil.getMethod(entity.getClass(), ChainConstant.DB_TENANT_KEY_GET_METHOD);
			String tenantId = String.valueOf(getTenantId.invoke(entity));
			if (ObjectUtil.isEmpty(tenantId)) {
				Method setTenantId = ClassUtil.getMethod(entity.getClass(), ChainConstant.DB_TENANT_KEY_SET_METHOD, String.class);
