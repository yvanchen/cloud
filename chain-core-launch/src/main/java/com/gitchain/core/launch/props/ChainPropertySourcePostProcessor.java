package com.gitchain.core.launch.props;
 ...
 */
@Slf4j
public class ChainPropertySourcePostProcessor implements BeanFactoryPostProcessor, InitializingBean, Ordered {
	private final ResourceLoader resourceLoader;
	private final List<PropertySourceLoader> propertySourceLoaders;

	public ChainPropertySourcePostProcessor() {
	...
	public void postProcessBeanFactory(ConfigurableListableBeanFactory beanFactory) throws BeansException {
		log.info("ChainPropertySourcePostProcessor process @ChainPropertySource bean.");
		Map<String, Object> beansWithAnnotation = beanFactory.getBeansWithAnnotation(ChainPropertySource.class);
		Set<Map.Entry<String, Object>> beanEntrySet = beansWithAnnotation.entrySet();
		// 没有 @YmlPropertySource 注解，跳出
		if (beanEntrySet.isEmpty()) {
			log.warn("Not found @ChainPropertySource on spring bean class.");
			return;
		}
		// 组装资源
		...
		for (Map.Entry<String, Object> entry : beanEntrySet) {
			...
			ChainPropertySource propertySource = AnnotationUtils.getAnnotation(beanClass, ChainPropertySource.class);
	...
	private static void loadPropertySource(String location, Resource resource,
		...
		if (resource.exists()) {
			String name = "chainPropertySource: [" + location + "]";
			try {
	...
	public void afterPropertiesSet() throws Exception {
		log.info("ChainPropertySourcePostProcessor init.");
	}
