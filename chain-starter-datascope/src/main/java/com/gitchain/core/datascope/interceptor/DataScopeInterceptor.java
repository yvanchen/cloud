package com.gitchain.core.datascope.interceptor;
...
import com.gitchain.core.datascope.annotation.DataAuth;
...
public class DataScopeInterceptor implements QueryInterceptor {
	...
	public void intercept(Executor executor, MappedStatement ms, Object parameter, RowBounds rowBounds, ResultHandler resultHandler, BoundSql boundSql) {
		...
		//未取到用户则放行
		ChainUser chainUser = AuthUtil.getUser();
		if (chainUser == null) {
			return;
		}
		...
		//获取数据权限规则对应的筛选Sql
		String sqlCondition = dataScopeHandler.sqlCondition(mapperId, dataScope, chainUser, originalSql);
