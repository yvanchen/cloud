 ...
 * <p>
 *     url: /v1/user/{id}
 *     header: Accept application/vnd.chain.VERSION+json
 * </p>
 *
 * 注意：c 代表客户端版本
 *
 * @author git
 */
public class ChainRequestMappingHandlerMapping extends RequestMappingHandlerMapping {
	...
	private RequestMappingInfo getApiVersionMappingInfo(Method method, Class<?> handlerType) {
		...
		// 如果url版本不存在
		if (nonUrlVersion) {
			String vsersionMediaTypes = new ChainMediaType(apiVersion.value()).toString();
